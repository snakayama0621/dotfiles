...
# 推奨
brew install fd lazygit yazi im-select deno
...
LuaやPythonなどのコードファイルを開くと、LSP関連プラグインが読み込まれ、サーバーの自動インストールが始まります。その後、以下で状態を確認します。

```vim
:Mason
```

3. **フォーマッター・リンターの初回インストール:**

起動時の自動導入は無効にしているため、コードファイルを開いた後、初回に以下を実行します。

```vim
:MasonToolsInstall
```

完了は `:Mason` で確認できます。`lsp.lua` の導入対象を追加した場合も、このコマンドを再実行してください。
...
設定の回帰テストは、dotfilesのルートから `npm run test:nvim` で実行できます。プラグインの導入や外部接続は行わず、一時環境でバッファ操作と設定のコールバックを検証します。

インストール済みプラグインとの連携（DAPの識別子解決、Copilotの改名時の接続解除）も検証する場合は、以下を実行します。LSPの通信先はテスト内で置き換えます。

```bash
DOTFILES_NVIM_PLUGINS="$HOME/.local/share/nvim/lazy" npm run test:nvim
```
...
| キー | モード | 説明 |
|------|--------|------|
| `<C-l>` | インサート | Copilot提案を受け入れ |
| `<C-;>` | インサート | 単語単位で受け入れ |
| `<C-y>` | インサート | 行単位で受け入れ |
| `<M-]>` | インサート | 次の提案 |
| `<M-[>` | インサート | 前の提案 |
| `<C-]>` | インサート | 提案を却下 |
| `<C-CR>` | インサート | 候補一覧パネルを開く |
...
| `<leader>Dv` | 垂直分割でコンテナ内コマンド実行 |
| `<leader>Dh` | 水平分割でコンテナ内コマンド実行 |
...
- **telescope-fzf-native.nvim** - Telescopeのfzfネイティブソーター
...
- **mason-tool-installer.nvim** - フォーマッター・リンターの自動インストール

#### フォーマット・Lint・デバッグ
- **conform.nvim** - フォーマッター統合
- **nvim-lint** - リンター統合
- **nvim-dap** - デバッグアダプター統合
- **nvim-dap-ui** - デバッグUI
- **nvim-dap-virtual-text** - デバッグ情報のインライン表示
- **mason-nvim-dap.nvim** - DAPアダプター管理
...
- **nvim-ts-autotag** - HTML/XMLタグの自動補完
- **nvim-surround** - 囲み文字操作
- **Comment.nvim** - コメント操作
- **nvim-colorizer.lua** - カラーコード表示
- **ccc.nvim** - カラーピッカー
- **im-select.nvim** - 入力ソース切り替え

#### AI補完
- **copilot.lua** - GitHub Copilot統合（インライン提案・パネル表示）
...
- **obsidian.nvim** - Obsidian vault統合
...
│       ├── lint.lua         # リンター
│       ├── dap.lua          # デバッガー
│       ├── devcontainer.lua # Dev Container統合
│       └── *.lua            # その他プラグイン設定
└── README.md                # このファイル
```
