...
# Codexローカル設定（生成物）
CODEX_USER_CONFIG_TEMPLATE="$DOTFILE_DIR/.codex/user-config.toml.template"
CODEX_USER_CONFIG="$DOTFILE_DIR/.codex/user-config.toml"
CODEX_USER_CONFIG_LOCAL="$DOTFILE_DIR/.codex/user-config.local.toml"
CODEX_USER_CONFIG_MARKER="# Generated by link.sh from .codex/user-config.toml.template. Do not edit directly."

# Claude設定（マシンごとに変わるためGit管理外。example から初回作成）
CLAUDE_SETTINGS="$DOTFILE_DIR/.claude/settings.json"
CLAUDE_SETTINGS_EXAMPLE="$DOTFILE_DIR/.claude/settings.example.json"
...
LINKS=(
  ...
  "$HOME/.config/herdr/config.toml:$DOTFILE_DIR/herdr/config.toml"
  ...
  "$HOME/.claude/hooks:$DOTFILE_DIR/.claude/hooks"
...
# テンプレート変数を現在のマシン向けに展開する
render_codex_template_line() {
  local line="$1"
  line="${line//\{\{HOME\}\}/$HOME}"
  line="${line//\{\{DOTFILE_DIR\}\}/$DOTFILE_DIR}"
  printf '%s\n' "$line"
}

# Codexの絶対パス・マシン固有値を現在の環境へ同期
setup_codex_user_config() {
  if [ ! -f "$CODEX_USER_CONFIG_TEMPLATE" ]; then
    log_warning "Codex user config template not found: $CODEX_USER_CONFIG_TEMPLATE"
    return 1
  fi

  if [ "$DRY_RUN" = true ]; then
    log_info "[DRY-RUN] Would render Codex config: $CODEX_USER_CONFIG"
    if [ -f "$CODEX_USER_CONFIG" ] && ! grep -Fxq "$CODEX_USER_CONFIG_MARKER" "$CODEX_USER_CONFIG"; then
      log_warning "[DRY-RUN] Would backup existing non-generated Codex config"
    fi
    if [ -f "$CODEX_USER_CONFIG_LOCAL" ]; then
      log_info "[DRY-RUN] Would append local Codex config: $CODEX_USER_CONFIG_LOCAL"
    fi
    return 0
  fi

  if [ -f "$CODEX_USER_CONFIG" ] && ! grep -Fxq "$CODEX_USER_CONFIG_MARKER" "$CODEX_USER_CONFIG"; then
    local backup="${CODEX_USER_CONFIG}.backup.$(date +%Y%m%d_%H%M%S)"
    mv "$CODEX_USER_CONFIG" "$backup"
    log_warning "Backed up existing Codex config: $CODEX_USER_CONFIG -> $backup"
  fi

  {
    while IFS= read -r line || [ -n "$line" ]; do
      render_codex_template_line "$line"
    done < "$CODEX_USER_CONFIG_TEMPLATE"

    if [ -f "$CODEX_USER_CONFIG_LOCAL" ]; then
      printf '\n# Local additions from .codex/user-config.local.toml\n'
      cat "$CODEX_USER_CONFIG_LOCAL"
    fi
  } > "$CODEX_USER_CONFIG"

  log_success "Rendered Codex user config: $CODEX_USER_CONFIG"
}

# Claude設定が無ければ example からコピーする（既存は上書きしない）
setup_claude_settings() {
  if [ -f "$CLAUDE_SETTINGS" ]; then
    log_info "Claude settings already exists: $CLAUDE_SETTINGS"
    return 0
  fi

  if [ ! -f "$CLAUDE_SETTINGS_EXAMPLE" ]; then
    log_warning "Claude settings example not found: $CLAUDE_SETTINGS_EXAMPLE"
    return 1
  fi

  if [ "$DRY_RUN" = true ]; then
    log_info "[DRY-RUN] Would create Claude settings from example: $CLAUDE_SETTINGS"
    return 0
  fi

  cp "$CLAUDE_SETTINGS_EXAMPLE" "$CLAUDE_SETTINGS"
  log_success "Created Claude settings from example: $CLAUDE_SETTINGS"
}
...
create_link() {
  ...
  if [ ! -e "$source" ]; then
    if [ "$DRY_RUN" = true ] && { [ "$source" = "$CODEX_USER_CONFIG" ] || [ "$source" = "$CLAUDE_SETTINGS" ]; }; then
      log_info "[DRY-RUN] Source will be generated: $source"
      return 0
    fi
...
# Codex user config の生成（失敗してもリンク作成は続行）
setup_codex_user_config || log_warning "Codex user config setup skipped. Create $CODEX_USER_CONFIG manually."
echo ""

# Claude settings の初回作成（失敗してもリンク作成は続行）
setup_claude_settings || log_warning "Claude settings setup skipped. Create $CLAUDE_SETTINGS manually."
echo ""
